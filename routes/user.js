...
router.post('/create', authenticateJwt, async (req, res) => {
  try {
      const { title, description, status, published, deadline, tags } = req.body;
      const Id = req.user.username;
      console.log(tags);

      // Iterate through the tags array
      for (let i = 0; i < tags.length; i++) {
          // Find user with the current tag (which is a username)
          const userWithMatchingTag = await User.findOne({ username: tags[i] });
          // If a user with the matching username is found
          if (userWithMatchingTag) {
              const task = new Task({
                  title,
                  description,
                  status,
                  ...
                  Id,
                  user_Id: userWithMatchingTag.username // Assign the user's ID to the task's user_Id field
              });

              // Save the task after associating it with the user
              await task.save();
          }
      }
